...
function createScriptSandbox(options = {}) {
  ...
  // console.group/groupEnd: 很多 LX 社区音源脚本用 console.group() 分组日志，
  // 沙箱里没有这些方法会抛 TypeError 导致整个请求失败。转发到 log 即可。
  consoleObject.group = wrapFn((...args) => { onLog('log', args.map(fmtLogArg).join(' ')) }, 'none')
  consoleObject.groupEnd = wrapFn(() => {}, 'none')
