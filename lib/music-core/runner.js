...
// 网络相关错误码：属于 I/O 失败，不会破坏进程状态，仅记录日志不退出。
// 音源脚本常因域名失效 / 网络不通抛出未捕获的 DNS/连接错误，若直接退出子进程，
// 会导致所有音源（含正常的）一起熔断不可用。
const NETWORK_ERROR_CODES = new Set([
  'ENOTFOUND', 'ECONNREFUSED', 'ECONNRESET', 'ETIMEDOUT',
  'EHOSTUNREACH', 'ENETUNREACH', 'EPIPE', 'EAI_AGAIN',
  'ECONNABORTED', 'ENOTCONN', 'ESHUTDOWN', 'EHOSTDOWN',
  'EADDRINUSE', 'EADDRNOTAVAIL',
])

// 子进程内任何未捕获异常都不应波及主进程；退出后由主进程按重启策略恢复。
// 但网络 I/O 错误仅记录日志不退出，避免单个坏音源拖垮全部音源。
process.on('uncaughtException', (err) => {
  const code = err && err.code
  if (code && NETWORK_ERROR_CODES.has(code)) {
    console.error('[runner] 网络错误（已忽略，进程继续）:', err && err.message)
    return
  }
...
// 未捕获的 Promise rejection 同理：网络错误容忍，其他退出
process.on('unhandledRejection', (err) => {
  const code = err && err.code
  if (code && NETWORK_ERROR_CODES.has(code)) {
    console.error('[runner] 网络错误 rejection（已忽略）:', err && err.message)
    return
  }
  console.error('[runner] unhandled rejection，进程将退出:', err && err.message)
  process.exit(1)
})
